...
{
    int res = -3;  // Inicializamos en el caso que se rompa la escalera 

    // Verificamos que la carta forma una escalera
    if((card_number(snd) == card_number(fst) - 1 && card_number(fst) > 1) || (card_number(snd) == card_number(fst) + 1 && card_number(fst)<= 12)){
        // Caso donde comienza la escalera
        if(!isFlush){
            res = 5;
        } else {  // Ya habia una escalera comenzada
            // Caso si sigue el mismo sentido 
            if(upwardsFlush){  // EL sentido es ascedente
                if(card_number(snd) == card_number(fst) + 1){
                    res = 3;
                } else {
                    res = 5;
                }
            } else {  // EL sentido es descedente
                if(card_number(snd) == card_number(fst) - 1){
                    res = 3;
                } else {
                    res = 5;
    ...
    if(card_number(snd) == card_number(fst)){
        res = 7;
    }
    return res;
}
