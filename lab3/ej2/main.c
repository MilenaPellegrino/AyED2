...
int main(int argc, char *argv[]) {
    ...
    printf("Menor temperatura general registrada: %d", low_min_temp(array));
    printf("\n");
    
    int output[YEARS];
    procedimiento(array, output);
    for(unsigned int i =0; i<YEARS; i++){
           printf("Year: %d, Maxima Temperatura: %d", i + FST_YEAR, output[i]);
    }
    printf("\n");
    month_t outputm[YEARS];
    month_more_rain(array, outputm);
    for(int i=0; i<YEARS; i++){
         printf("Year: %d, Mes con mas precipitaciones: %d", i+FST_YEAR, outputm[i]);
    }
