...
Weather weather_from_file(FILE* file)
{
    Weather weather;
    /* Completar aqui */
    int r = fscanf(file, "%d %d %d %u %u %u", &weather._average_temp, &weather._max_temp, &weather._min_temp, &weather._pressure, &weather._moisture, &weather._rainfall);
    if(r!= AMOUNT_OF_WEATHER_VARS){
      fprintf(stderr, "Formato del array invalido");
      exit(EXIT_FAILURE);
    }
    return weather;
}
